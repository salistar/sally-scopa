/**
 * @file (tabs)/index.tsx
 * @description Home Scopa. 4 actions principales : Créer, Rejoindre, vs Bot
...
export default function HomeScreen() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setUser(u);
      log.bin('GET /challenges/daily/scopa');
      const ch = await api.getDailyChallenge('scopa');
      if (ch) {
        log.bout('200 /challenges/daily/scopa', { title: ch.title, reward: ch.rewardCoins });
        setChallenge(ch);
      }
  ...
  const handleChallenge = async () => {
    log.bin('POST /challenges/daily/scopa/matchmake');
    try {
      const room = await api.joinDailyChallenge('scopa');
