 ...
 * @description Leaderboard Scopa avec 3 portées : Mondial / Pays / Ville.
 * Données réelles depuis /leaderboards/scopa?scope=<...>, auto-détection
...
export default function LeaderboardScreen() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      log.bin(`GET /leaderboards/scopa`, { scope, filter });
      const r = await api.getLeaderboardScoped('scopa', filter, scope, 50);
      log.bout('200 /leaderboards/scopa', `${r.entries.length} entrées`);
