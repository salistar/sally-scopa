 ...
 * @description Profil Scopa avec vraies valeurs (stats/coins/achievements/
...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // 2) /leaderboards/scopa/my-rank : best-effort
      try {
        log.bin('GET /leaderboards/scopa/my-rank');
        const r = await api.getMyRank('scopa', 'season');
        ...
        const msg = e?.message ?? e?.toString?.() ?? 'unknown';
        const status = e?.status ?? '?';
        if (status === 404) log.explain('pas encore de rang scopa pour ce joueur');
